...
void wifi_sniffer_packet_handler(uint8_t *buff, uint16_t len)
{
	...
	const wifi_pkt_mgmt_t  *ppktSDKbuf = (wifi_pkt_mgmt_t *)buff;							// der zeigt auf die SDK data ( ssid des Device )
	...
	if (frame_ctrl->type == WIFI_PKT_MGMT && frame_ctrl->subtype == BEACON)
	{
		...
		//soll uns die ssid des calling device geben

		if (frame_ctrl->type == WIFI_PKT_MGMT && frame_ctrl->subtype == PROBE_REQ)
		{
			//Serial.print("ProbeReq: "); Serial.print(ppktSDKbuf->buf[25]);
			//Serial.printf("%s", ssid);
			uint8_t SSID_length = ppktSDKbuf->buf[25];
			for (uint16_t i = 26; i < 112 && i < 26 + SSID_length; i++) {
				Serial.write(ppktSDKbuf->buf[i]);
			}
		}
	}
}
